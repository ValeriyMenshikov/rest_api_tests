...
def test_put_v1_account_password(logic, prepare_user):
    ...
    logic.account_helper.reset_user_password(
        login=login, email=email, headers=access_token
    )
    reset_token = logic.mailhog_helper.get_token_by_login(
        login=login, token_type=TokenType.RESET_PASSWORD
    )
